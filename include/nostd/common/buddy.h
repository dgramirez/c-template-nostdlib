...
usz buddy_sizeof(usz memory_size);

/*
 * Returns the size of a buddy required to manage a block of the specified size
 * using a non-default alignment.
 */
usz buddy_sizeof_alignment(usz memory_size, usz alignment);

/* Initializes a binary buddy memory allocator at the specified location */
struct buddy *buddy_init(unsigned char *at, unsigned char *main, usz memory_size);

/* Initializes a binary buddy memory allocator at the specified location using a non-default alignment */
struct buddy *buddy_init_alignment(unsigned char *at, unsigned char *main, usz memory_size, usz alignment);
 ...
 */
struct buddy *buddy_embed(unsigned char *main, usz memory_size);
 ...
 */
struct buddy *buddy_get_embed_at(unsigned char *main, usz memory_size);
 ...
 * The arena's capacity is reduced to account for the allocator metadata.
 */
struct buddy *buddy_embed_alignment(unsigned char *main, usz memory_size, usz alignment);

/*
 * Returns the address of a previously-created buddy allocator at the arena.
 * Use to get a new handle to the allocator when the arena is moved or copied.
 */
struct buddy *buddy_get_embed_at_alignment(unsigned char *main, usz memory_size, usz alignment);
 ...
 * are preserved.
 */
struct buddy *buddy_resize(struct buddy *buddy, usz new_memory_size);

/* Tests if the arena can be shrunk in half */
i32 buddy_can_shrink(struct buddy *buddy);

/* Tests if the arena is completely empty */
i32 buddy_is_empty(struct buddy *buddy);

/* Tests if the arena is completely full */
i32 buddy_is_full(struct buddy *buddy);

/* Reports the arena size */
usz buddy_arena_size(struct buddy *buddy);

/* Reports the arena's free size. Note that this is (often) not a continuous size
   but the sum of all free slots in the buddy. */
usz buddy_arena_free_size(struct buddy *buddy);
...
void *buddy_malloc(struct buddy *buddy, usz requested_size);

/* Use the specified buddy to allocate zeroed memory. See calloc. */
void *buddy_calloc(struct buddy *buddy, usz members_count, usz member_size);

/* Realloc semantics are a joke. See realloc. */
void *buddy_realloc(struct buddy *buddy, void *ptr, usz requested_size, i32 ignore_data);

/* Realloc-like behavior that checks for overflow. See reallocarray */
void *buddy_reallocarray(struct buddy *buddy, void *ptr,
    usz members_count, usz member_size, i32 ignore_data);
...
enum buddy_safe_free_status buddy_safe_free(struct buddy *buddy, void *ptr, usz requested_size);
...
usz buddy_alloc_size(struct buddy *buddy, void *ptr);
...
void buddy_reserve_range(struct buddy *buddy, void *ptr, usz requested_size);

/* Release a reserved memory range. Unsafe, this can mess up other allocations if called with wrong parameters! */
void buddy_unsafe_release_range(struct buddy *buddy, void *ptr, usz requested_size);
 ...
 */
void *buddy_walk(struct buddy *buddy, void *(fp)(void *ctx, void *addr, usz slot_size, usz allocated), void *ctx);
 ...
 */
void buddy_enable_change_tracking(struct buddy* buddy, void* context, void (*tracker) (void*, unsigned char*, usz));
#endif
...
#define BUDDY_ALLOC_ALIGN (sizeof(usz) * 8)
#endif
...
struct buddy_change_tracker {
    void* context;
    void (*tracker) (void*, unsigned char*, usz);
};
...
static usz buddy_tree_sizeof(u8 order);

/* Initializes a buddy allocation tree at the specified location */
static struct buddy_tree *buddy_tree_init(unsigned char *at, u8 order);

/* Indicates whether this is a valid position for the tree */
static i32 buddy_tree_valid(struct buddy_tree *t, struct buddy_tree_pos pos);

/* Returns the order of the specified buddy allocation tree */
static u8 buddy_tree_order(struct buddy_tree *t);
 ...
 */
static void buddy_tree_resize(struct buddy_tree *t, u8 desired_order);
...
static inline usz buddy_tree_depth(struct buddy_tree_pos pos);
...
static usz buddy_tree_index(struct buddy_tree_pos pos);
...
static i32 buddy_tree_interval_contains(struct buddy_tree_interval outer,
...
static usz buddy_tree_status(struct buddy_tree *t, struct buddy_tree_pos pos);
...
static struct buddy_tree_pos buddy_tree_find_free(struct buddy_tree *t, u8 depth);

/* Tests if the indicated position is available for allocation */
static i32 buddy_tree_is_free(struct buddy_tree *t, struct buddy_tree_pos pos);

/* Tests if the tree can be shrank in half */
static i32 buddy_tree_can_shrink(struct buddy_tree *t);
...
/* Implementation defined */
static void buddy_tree_debug(struct buddy_tree *t, struct buddy_tree_pos pos, usz start_size);
...
static usz bitset_sizeof(usz elements);
...
static inline struct bitset_range bitset_range(usz from_pos, usz to_pos);
...
static usz bitset_count_range(unsigned char *bitset, struct bitset_range range);

static inline void bitset_set(unsigned char *bitset, usz pos);

static inline void bitset_clear(unsigned char *bitset, usz pos);

static inline i32 bitset_test(const unsigned char *bitset, usz pos);

static void bitset_shift_left(unsigned char *bitset, usz from_pos, usz to_pos, usz by);

static void bitset_shift_right(unsigned char *bitset, usz from_pos, usz to_pos, usz by);
...
/* Implementation defined */
void bitset_debug(unsigned char *bitset, usz length);
...
static usz highest_bit_position(usz value);

/* Returns the nearest larger or equal power of two */
static inline usz ceiling_power_of_two(usz value);

/* Return two to the power of order */
static inline usz two_to_the_power_of(usz order);
...
static inline usz integer_square_root(usz f);
...
static unsigned int is_valid_alignment(usz alignment);
static usz buddy_tree_order_for_memory(usz memory_size, usz alignment);
static usz depth_for_size(struct buddy *buddy, usz requested_size);
static inline usz size_for_depth(struct buddy *buddy, usz depth);
...
static usz buddy_effective_memory_size(struct buddy *buddy);
static usz buddy_virtual_slots(struct buddy *buddy);
static void buddy_toggle_virtual_slots(struct buddy *buddy, unsigned int state);
static void buddy_toggle_range_reservation(struct buddy *buddy, void *ptr, usz requested_size, unsigned int state);
static struct buddy *buddy_resize_standard(struct buddy *buddy, usz new_memory_size);
static struct buddy *buddy_resize_embedded(struct buddy *buddy, usz new_memory_size);
static i32 buddy_is_free(struct buddy *buddy, usz from);
static struct buddy_embed_check buddy_embed_offset(usz memory_size, usz alignment);
static struct buddy_tree_pos deepest_position_for_offset(struct buddy *buddy, usz offset);
...
usz buddy_sizeof_alignment(usz memory_size, usz alignment) {
    ...
    return sizeof(struct buddy) + buddy_tree_sizeof((u8)buddy_tree_order);
}
...
struct buddy *buddy_init_alignment(unsigned char *at, unsigned char *main, usz memory_size,
        usz alignment) {
    usz at_alignment, main_alignment, buddy_size, buddy_tree_order;
    struct buddy *buddy;
    ...
    main_alignment = ((uintptr_t) main) % BUDDY_ALIGNOF(usz);
    if (main_alignment != 0) {
        return NULL;
    }
    ...
    buddy_tree_init((unsigned char *)buddy + sizeof(*buddy), (u8) buddy_tree_order);
...
static struct buddy *buddy_resize_standard(struct buddy *buddy, usz new_memory_size) {
    ...
    buddy_tree_resize(buddy_tree(buddy), (u8) new_buddy_tree_order);
...
static struct buddy *buddy_resize_embedded(struct buddy *buddy, usz new_memory_size) {
    ...
    memmoveu(buddy_destination, resized, check_result.buddy_size);
...
usz buddy_arena_free_size(struct buddy *buddy) {
    ...
    usz tree_order = buddy_tree_order(tree);

    struct buddy_tree_walk_state state = buddy_tree_walk_state_root();
    do {
        usz pos_status = buddy_tree_status(tree, state.current_pos);
...
static usz buddy_tree_order_for_memory(usz memory_size, usz alignment) {
    usz blocks = memory_size / alignment;
...
void *buddy_malloc(struct buddy *buddy, usz requested_size) {
    ...
    tree = buddy_tree(buddy);
    pos = buddy_tree_find_free(tree, (u8) target_depth);
...
void *buddy_realloc(struct buddy *buddy, void *ptr, usz requested_size, i32 ignore_data) {
    ...
    usz current_depth, target_depth;
    ...
    new_pos = buddy_tree_find_free(tree, (u8) target_depth);
    ...
    if (! ignore_data) {
        ...
        memmoveu(destination, source, size_for_depth(buddy,
...
void *buddy_walk(struct buddy *buddy,
        void *(fp)(void *ctx, void *addr, usz slot_size, usz allocated),
        void *ctx) {
    unsigned char *main;
    usz effective_memory_size, tree_order, pos_status, pos_size;
    struct buddy_tree *tree;
    unsigned char *addr;
    ...
    do {
        ...
        if (((usz)(addr - main) + pos_size) > buddy->memory_size) {
            /*
...
static usz depth_for_size(struct buddy *buddy, usz requested_size) {
    usz depth, effective_memory_size;
    if (requested_size < buddy->alignment) {
        requested_size = buddy->alignment;
    }
    depth = 1;
...
static usz buddy_virtual_slots(struct buddy *buddy) {
    usz memory_size = buddy->memory_size;
    usz effective_memory_size = buddy_effective_memory_size(buddy);
...
static unsigned char *address_for_position(struct buddy *buddy, struct buddy_tree_pos pos) {
    usz block_size = size_for_depth(buddy, buddy_tree_depth(pos));
    usz addr = block_size * buddy_tree_index(pos);
    return buddy_main(buddy) + addr;
}

static struct buddy_tree_pos deepest_position_for_offset(struct buddy *buddy, usz offset) {
    usz index = offset / buddy->alignment;
...
static void buddy_toggle_virtual_slots(struct buddy *buddy, unsigned int state) {
    usz delta, memory_size, effective_memory_size;
    struct buddy_tree *tree;
    struct buddy_tree_pos pos;
    ...
    while (delta) {
        usz current_pos_size = size_for_depth(buddy, buddy_tree_depth(pos));
...
static i32 buddy_is_free(struct buddy *buddy, usz from) {
    ...
    struct buddy_tree_pos pos;
    usz effective_memory_size, virtual_slots, to;
...
static struct buddy_embed_check buddy_embed_offset(usz memory_size, usz alignment) {
    usz buddy_size, offset;
    struct buddy_embed_check check_result;

    memsetu(&check_result, 0, sizeof(check_result));
...
static inline usz size_for_order(u8 order, u8 to);
static inline usz buddy_tree_index_internal(struct buddy_tree_pos pos);
static struct buddy_tree_pos buddy_tree_leftmost_child_internal(usz tree_order);
static struct internal_position buddy_tree_internal_position_order(
    usz tree_order, struct buddy_tree_pos pos);
static struct internal_position buddy_tree_internal_position_tree(
    struct buddy_tree *t, struct buddy_tree_pos pos);
static void buddy_tree_grow(struct buddy_tree *t, u8 desired_order);
static void buddy_tree_shrink(struct buddy_tree *t, u8 desired_order);
static void update_parent_chain(struct buddy_tree *t, struct buddy_tree_pos pos,
    struct internal_position pos_internal, usz size_current);
...
static inline usz buddy_tree_size_for_order(struct buddy_tree *t, u8 to);
static void write_to_internal_position(struct buddy_tree* t, struct internal_position pos, usz value);
static usz read_from_internal_position(unsigned char *bitset, struct internal_position pos);
static inline unsigned char compare_with_internal_position(unsigned char *bitset, struct internal_position pos, usz value);

#ifdef BUDDY_EXPERIMENTAL_CHANGE_TRACKING
static inline void buddy_tree_track_change(struct buddy_tree* t, unsigned char* addr, usz length);
#endif /* BUDDY_EXPERIMENTAL_CHANGE_TRACKING */
...
static inline struct internal_position buddy_tree_internal_position_order(
    ...
    usz total_offset, local_index;

    p.local_offset = tree_order - buddy_tree_depth(pos) + 1;
    total_offset = size_for_order((u8) tree_order, (u8) p.local_offset);
...
static inline struct internal_position buddy_tree_internal_position_tree(
    ...
    total_offset = buddy_tree_size_for_order(t, (u8) p.local_offset);
    local_index = buddy_tree_index_internal(pos);
    p.bitset_location = total_offset + (p.local_offset * local_index);
    return p;
}
...
static void buddy_tree_grow(struct buddy_tree *t, u8 desired_order) {
    ...
    while (desired_order > t->order) {
        ...
        while(current_order) {
            ...
            usz node_count = two_to_the_power_of(current_order - 1u);
...
static void buddy_tree_shrink(struct buddy_tree *t, u8 desired_order) {
    usz current_order, next_order, node_count;
...
static inline usz buddy_tree_index_internal(struct buddy_tree_pos pos) {
    ...
    usz mask = two_to_the_power_of(pos.depth - 1u);
    usz result = pos.index & ~mask;
    return result;
}
...
static void buddy_tree_populate_size_for_order(struct buddy_tree *t) {
    usz bitset_offset = bitset_sizeof(size_for_order(t->order, 0));
    if (bitset_offset % sizeof(usz)) {
        bitset_offset += (bitset_offset % sizeof(usz));
    }
    t->size_for_order_offset = bitset_offset / sizeof(usz);
    t->size_for_order_offset++;
    for (usz i = 0; i <= t->order; i++) {
        *((usz *)(((unsigned char *) t) + sizeof(*t)) + t->size_for_order_offset + i) = size_for_order(t->order, (u8) i);
    }
}

static inline usz buddy_tree_size_for_order(struct buddy_tree *t,
         u8 to) {
    return *((usz *)(((unsigned char *) t) + sizeof(*t)) + t->size_for_order_offset + to);
}
...
static void update_parent_chain(struct buddy_tree *t, struct buddy_tree_pos pos,
        struct internal_position pos_internal, usz size_current) {
    usz size_sibling, size_parent, target_parent;
...
static struct buddy_tree_pos buddy_tree_find_free(struct buddy_tree *t, u8 target_depth) {
    struct buddy_tree_pos current_pos, left_pos, right_pos;
    u8 target_status;
    usz current_depth, right_status;
...
static i32 buddy_tree_is_free(struct buddy_tree *t, struct buddy_tree_pos pos) {
    ...
    while(buddy_tree_valid(t, pos)) {
        struct internal_position internal = buddy_tree_internal_position_tree(t, pos);
        usz value = read_from_internal_position(buddy_tree_bits(t), internal);
        if (value) {
...
unsigned int buddy_tree_check_invariant(struct buddy_tree *t, struct buddy_tree_pos pos) {
    ...
    do {
        ...
        usz current_status = read_from_internal_position(buddy_tree_bits(t), current_internal);
        usz left_child_status = buddy_tree_status(t, buddy_tree_left_child(pos));
        usz right_child_status = buddy_tree_status(t, buddy_tree_right_child(pos));
        unsigned int violated = 0;

        if (left_child_status || right_child_status) {
            usz min = left_child_status <= right_child_status
...
static unsigned char buddy_tree_fragmentation(struct buddy_tree *t) {
    ...
    u8 tree_order;
    usz root_status, quality, total_free_size, virtual_size, quality_percent;
    struct buddy_tree_walk_state state;
    ...
    state = buddy_tree_walk_state_root();
    do {
        usz pos_status = buddy_tree_status(t, state.current_pos);
        if (pos_status == 0) {
            /* Empty node, process */
            virtual_size = two_to_the_power_of((tree_order - state.current_pos.depth) % ((sizeof(usz) * 8)-1));
...
usz bitset_sizeof(usz elements) {
    return ((elements) + 8 - 1u) / 8;
}

static u8 bitset_index_mask[8] = {1, 2, 4, 8, 16, 32, 64, 128};
...
static const u8 bitset_char_mask[8][8] = {
...
static inline struct bitset_range bitset_range(usz from_pos, usz to_pos) {
    ...
    range.from_bucket = from_pos / 8;
    range.to_bucket = to_pos / 8;

    range.from_index = from_pos % 8;
    range.to_index = to_pos % 8;
    return range;
}
...
static void bitset_shift_right(unsigned char *bitset, usz from_pos, usz to_pos, usz by) {
    isz length = (isz) to_pos - (isz) from_pos;
    while (length >= 0) {
        usz at = from_pos + (usz) length;
        if (bitset_test(bitset, at)) {
            bitset_set(bitset, at+by);
        } else {
            bitset_clear(bitset, at+by);
...
static usz highest_bit_position(usz value) {
    usz result = 0;
    /* some other millennia when usz becomes 128-bit this will break :) */
#if SIZE_MAX == 0xFFFFFFFFFFFFFFFF
    const usz all_set[] = {4294967295, 65535, 255, 15, 7, 3, 1};
    const usz count[] = {32, 16, 8, 4, 2, 1, 1};
#elif SIZE_MAX == 0xFFFFFFFF
    const usz all_set[] = {65535, 255, 15, 7, 3, 1};
    const usz count[] = {16, 8, 4, 2, 1, 1};
#else
#error Unsupported platform
#endif

    for (usz i = 0; i < (sizeof all_set / sizeof *all_set); i++) {
        if (value >= all_set[i]) {
            value >>= count[i];
            result += count[i];
        }
...
static inline usz integer_square_root(usz op) {
    /* by Martin Guy, 1985 - http://medialab.freaknet.org/martin/src/sqrt/ */
    usz result = 0;
    usz cursor = (SIZE_MAX - (SIZE_MAX >> 1)) >> 1; /* second-to-top bit set */
    while (cursor > op) {
        cursor >>= 2;
    }
